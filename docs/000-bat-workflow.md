...
Названия столбцов доски соответствуют названиям этапов развития задачи.  
Рабочий цикл начинается с этапа `In progress`, и заканчивается этапом `Done`.  
Столбцы `Backlog` и `Todo` не являются частью рабочего цикла.  
Они хранят списки задач, которые ещё не были приняты на реализацию.  
`Backlog`     - список идей и предложений.  
`Todo`        - список задач, отобранных для реализации.  
`In progress` - задачи, которые уже в стадии разработки.  
`Review`      - задача завершена, и находится в стадии code-review.  
`Done`        - задача полностью отработа, код уже в master'е.  
...
1. У нас есть общий список задач `backlog`, 
   ...
   Актуальные, по его мнению, задачи он перемещает в список `todo`.

2. `todo` - это список задач, которые были отобраны к исполнению.  
   ...
   В этом случае, его роль берёт на себя список `todo`.  

3. Один разработчик единовременно исполняет только одну задачу.  
   Пока одну не доделал, на другие не переключается.  

4. Завершив очередную задачу, разработчик принимает следующую из списка `todo`.  
...
13. Теперь разработчик готов взять следующую задачу из списка 'todo'
