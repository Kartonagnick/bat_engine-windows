...
5) [--clean](#--clean "удаление MakeFiles для всех указанных конфигураций")  
6) [--generate](#--generate "генерирует MakeFiles для всех указанных конфигураций")  
7) [--build](#--build "выполняет сборку MakeFiles для всех указанных конфигураций")  
8) [--install](#--install "выполняет исталляцию результатов сборки для всех указанных конфигураций")  
9) [--runTests](#--runTests "запускает тесты для указанных конфигураций")  
10) [--custom](#--custom "запуск произвольного батника")  
11) [--runIDE](#--runIDE "запускает IDE")  
...
- `configurations` Задаёт список [конфигураций][4], которые нужно удалить.  
  По умолчанию используется значение `АргументКоманды`  

`Примечание`  
Если `АргументКоманды` или `configurations` использовали значение `all`,
тогда весь каталог `eDIR_BUILD` будет удален целиком.  

Если заданы оба значения: и `АргументКоманды`, и `configurations`,
и при этом они отличны от `all`, тогда будут удалены каталоги сборок, 
соответствующие конфигурациям заданным в `configurations`  
(значение `АргументКоманды` будет проигнорированно)  
...
- `configurations` Здесь указывается одна единственная конфигурация для которой нужно запустить IDE.  
  Указывать нужно все 4ре тэга.  
  
`Примечание`  
При запуске QtCreator атрибут `configurations` можно не указывать.  
