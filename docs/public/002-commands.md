...
`Примечание` Для каждой конфигурации предварительное запускает --generate.  
...
`Примечание`  
В корневом каталоге исходного кода может находится файл 
...
формируется с учетом данных ограничений.  
